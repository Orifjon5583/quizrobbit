...
export default function Dashboard() {
  ...
  return <div className="animate-fade-in">
    ...
    <div className="mt-4 grid gap-2 sm:hidden">{results.slice(0, 5).map(r => { const total = r.correctCount + r.wrongCount; return <div className="card flex items-center justify-between gap-3" key={r.id}><div><p className="font-bold">{r.category}</p><p className="text-xs text-slate-500">{r.correctCount}/{total} to'g'ri · {r.level}</p></div><p className="text-xl font-black text-indigo-600">{r.score}</p></div>; })}{!results.length && <p className="card text-center text-sm text-slate-500">Hali natija mavjud emas.</p>}</div>
    <div className="mt-4 hidden card overflow-x-auto sm:block"><table className="w-full text-left text-sm"><thead><tr className="border-b dark:border-slate-700"><th className="p-3">Yo'nalish</th><th className="p-3">Natija</th><th className="p-3">Ball</th><th className="p-3">Daraja</th></tr></thead><tbody>{results.slice(0, 5).map(r => { const total = r.correctCount + r.wrongCount; return <tr className="border-b last:border-0 dark:border-slate-800" key={r.id}><td className="p-3 font-bold">{r.category}</td><td className="p-3">{r.correctCount}/{total}</td><td className="p-3">{r.score}</td><td className="p-3">{r.level}</td></tr>; })}</tbody></table>{!results.length && <p className="p-4 text-center text-slate-500">Hali natija mavjud emas.</p>}</div>
  </div>;
}
