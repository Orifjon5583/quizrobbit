...
export default function Quiz() {
  ...
  return <div className="mx-auto max-w-2xl animate-fade-in">
    ...
    <div className="card mt-4 sm:mt-5"><h1 className="text-lg font-black leading-relaxed sm:text-xl">{question.question}</h1><div className="mt-5 grid gap-3 sm:mt-6">{question.options.map((option, index) => <button disabled={busy} onClick={() => answer(option)} className="min-h-14 rounded-xl border border-slate-200 p-3.5 text-left text-sm font-medium active:scale-[.99] hover:border-indigo-500 hover:bg-indigo-50 disabled:opacity-60 sm:p-4 sm:text-base dark:border-slate-700 dark:hover:bg-indigo-950" key={option}><span className="mr-2 font-black text-indigo-600 sm:mr-3">{String.fromCharCode(65 + index)}.</span>{option}</button>)}</div></div>
    <p className="mt-4 text-center text-xs text-slate-500 sm:text-sm">Javob tanlangandan keyin ortga qaytib bo'lmaydi.</p>
  </div>;
}
