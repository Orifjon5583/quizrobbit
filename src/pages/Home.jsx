...
export default function Home() {
  const [catalog, setCatalog] = useState({ totalQuestions: 0, categories: [] });
  ...
  return <div className="animate-fade-in">
    <section className="grid items-center gap-6 py-5 sm:py-10 lg:grid-cols-2">
      <div>
        ...
        <p className="mt-4 max-w-xl text-base text-slate-600 sm:mt-5 sm:text-lg dark:text-slate-300">Bo'limlar siz kiritgan savollar bilan to'ldiriladi. Test mavjud savollar soniga qarab ishlaydi.</p>
      ...
      <div className="card grid grid-cols-3 gap-2 sm:gap-4">
        {[[String(catalog.totalQuestions || 0), "Savol"], [String(catalog.categories.length || categories.length), "Yo'nalish"], ["20s", "Har savol"]].map(([value, label]) => <div className="rounded-xl bg-indigo-50 p-3 text-center sm:rounded-2xl sm:p-5 dark:bg-indigo-950/50" key={label}><p className="text-2xl font-black text-indigo-600 sm:text-3xl">{value}</p><p className="text-xs text-slate-500 sm:text-sm">{label}</p></div>)}
