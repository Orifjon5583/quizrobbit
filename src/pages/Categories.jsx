...
export default function Categories() {
  return <div className="animate-fade-in">
    ...
    <p className="mt-2 text-sm text-slate-500 sm:text-base">Har bir quiz 25 ta savoldan iborat. Har savol uchun 20 soniya beriladi.</p>
    <div className="mt-5 grid grid-cols-2 gap-3 sm:mt-6 sm:gap-5 lg:grid-cols-4">
      {categories.map(({ name, icon: Icon, color, description }) => <Link className="card group flex min-h-40 flex-col hover:-translate-y-1 sm:min-h-0" to={`/quiz/${encodeURIComponent(name)}`} key={name}>
        ...
        <p className="mt-auto flex items-center justify-between pt-3 text-xs font-bold text-indigo-600 sm:mt-4 sm:pt-0 sm:text-sm">25 savol · 20s <ArrowRight size={16}/></p>
      </Link>)}
    </div>
  </div>;
}
